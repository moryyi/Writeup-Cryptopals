...
def break_repeating_key_xor(ciphertext):
    ...
    possible_key_lengths = sorted(average_distances, key=lambda x: x['avg distance'])
    for k in possible_key_lengths:
        print(k)
    print(len(possible_key_lengths))
    possible_key_lengths = possible_key_lengths[0]
    possible_plaintext = []
...
def main():
    with open('b64_cipher.txt') as input_file:
